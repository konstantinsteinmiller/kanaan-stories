...
<template lang="pug">
  nav(
    :class="[\
      's-bottom-nav fixed bottom-0 left-0 right-0 z-50 px-4 pt-2 pb-[max(env(safe-area-inset-bottom),0.75rem)]',\
      accent === 'yellow' ? 'accent-yellow' : 'accent-orange'\
    ]"
  )
    div(class="s-bottom-nav-inner relative mx-auto max-w-md flex items-center justify-around gap-1 rounded-3xl px-2 py-2")
      button(
        v-for="item in items"
        :key="item.id"
        type="button"
        ...
        :class="[\
          's-nav-btn group relative flex flex-1 flex-col items-center justify-center gap-1 cursor-pointer select-none touch-manipulation rounded-2xl py-2 px-1 transition-all duration-200 ease-out',\
        ...
        span(
          ...
          class="s-nav-glow pointer-events-none absolute inset-x-3 -bottom-1 h-8 rounded-full blur-xl"
        )
        ...
        span(
          :class="[\
            'relative z-10 text-[10px] md:text-[11px] font-bold tracking-wide transition-colors duration-200',\
            modelValue === item.id ? 's-nav-label-active' : 's-nav-label-inactive'\
          ]"
        ) {{ item.label }}
</template>
...
.s-bottom-nav
  background: linear-gradient(to top, rgba(31, 111, 168, 0.55), rgba(31, 111, 168, 0))
...
.s-bottom-nav-inner
  ...
  box-shadow: 0 0 0 1px rgba(255, 255, 255, 0.5), 0 -2px 24px -4px rgba(44, 62, 80, 0.25), 0 12px 32px -10px rgba(44, 62, 80, 0.4)
...
.accent-orange
  ...
  .s-nav-label-active
    color: #E67E22
    text-shadow: 0 0 10px rgba(230, 126, 34, 0.45)

  .s-nav-glow
    background: radial-gradient(closest-side, rgba(230, 126, 34, 0.55), rgba(230, 126, 34, 0))
...
.accent-yellow
  ...
  .s-nav-glow
    background: radial-gradient(closest-side, rgba(241, 196, 15, 0.6), rgba(241, 196, 15, 0))
</style>
